...
dim=2048           # Optional: only used if you want to pass matrix size dynamically
procs=1
...
mkdir -p "$bin_dir"#!/bin/bash
#PBS -N matmul_serial_mpi
#PBS -l select=1:ncpus=1:mem=12gb:mpiprocs=1
#PBS -l walltime=1:00:00
#PBS -q short_cpuQ

# ---- Configurable Section ----
src_file="matmul_serial.c"
dim=1024           # Optional: only used if you want to pass matrix size dynamically
procs=1
...
# ---- Paths ----
project_root="/home/saad.hussainshafi/HPC_uni_tn_2025/HPC_uni_tn_2025"
src_dir="$project_root/serial_mpi/src/main"
bin_dir="$project_root/serial_mpi/bin/mpi/$dim"
log_dir="$project_root/serial_mpi/logs/mpi/$dim"
...
mpicc -O3 -std=c99 -DN=$dim "$src_dir/$src_file" -o "$exe_file" -lm
if [ ! -f "$exe_file" ]; then
